...
func TestWordpressInstagramRepository_FindAll(t *testing.T) {
	...
	// テストデータを複数作成
	testData := []*domain.WordpressInstagram{
		{
			Name:               "Site A",
			WordpressDomain:    "https://sitea.example.com",
			WordpressSiteTitle: "Site A Title",
			InstagramID:        "111111111",
			InstagramName:      "usera",
			Memo:               "Site A memo",
			StartDate:          time.Date(2025, 1, 1, 0, 0, 0, 0, time.UTC),
			Status:             1,
		...
		{
			Name:               "Site B",
			WordpressDomain:    "https://siteb.example.com",
			WordpressSiteTitle: "Site B Title",
		...
		{
			Name:               "Site C",
			WordpressDomain:    "https://sitec.example.com",
			WordpressSiteTitle: "Site C Title",
			InstagramID:        "333333333",
			InstagramName:      "userc",
			Memo:               "Site C memo",
			StartDate:          time.Date(2025, 3, 1, 0, 0, 0, 0, time.UTC),
			Status:             2,
	...
	for _, wi := range testData {
		err := repo.Create(ctx, wi)
		assert.NoError(t, err)
	}
	...
	tests := []struct {
		...
		{
			name:      "Status=1でフィルター",
			filter:    WordpressInstagramFilter{Status: intPtr(1)},
			wantCount: 2,
		},
		{
			name:      "DeleteHash=trueでフィルター",
			filter:    WordpressInstagramFilter{DeleteHash: boolPtr(true)},
			wantCount: 1,
		},
		{
			name:      "CustomerType=1でフィルター",
			filter:    WordpressInstagramFilter{CustomerType: intPtr(1)},
			wantCount: 2,
		},
		{
			name:      "InstagramNameでフィルター",
			filter:    WordpressInstagramFilter{InstagramName: strPtr("userb")},
			wantCount: 1,
		},
		{
			name:      "WordpressDomainでフィルター",
			filter:    WordpressInstagramFilter{WordpressDomain: strPtr("https://sitea.example.com")},
			wantCount: 1,
		},
		{
			name:      "WordpressSiteTitleでフィルター",
			filter:    WordpressInstagramFilter{WordpressSiteTitle: strPtr("Site C Title")},
			wantCount: 1,
		},
		{
			name:      "Limit適用",
			filter:    WordpressInstagramFilter{Limit: intPtr(2)},
			wantCount: 2,
		},
	}
